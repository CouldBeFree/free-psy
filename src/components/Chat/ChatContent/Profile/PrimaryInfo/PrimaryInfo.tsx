...
const PrimaryInfo: FunctionComponent = () => {
  ...
  const photo = useSelector((state: RootState)  => state.authentication.currentUser?.photo);
  ...
  const onPhotoSelected: ChangeEventHandler<HTMLInputElement> = (e) => {
    if (e.target.files !== null && id !== undefined) {
      const file = e.target.files[0];
      dispatch(fetchSetPhoto({file, id}));
    }
  }
  ...
  return (
    <div className={style.primaryInfo}>
      ...
      <img src={photo ? photo : defaultPhoto} alt="User avatar" className={style.avatar}/>
      
      {isOwner && <label htmlFor="upload-button" className={classNames(buttonStyle.button, {[style.disabled]: isSubmitting})}>
      ...
      {!isOwner && <button className={classNames(buttonStyle.button, buttonStyle.shadow)}>
