...
const PrimaryInfo: FunctionComponent<PrimaryInfoProps> = ({userId}: PrimaryInfoProps) => {
  ...
  const writeMessage = () => {
    if (currentUser) {
      history.push(`/chat/${currentUser._id}`);
      dispatch(setCurrentRespondent(currentUser));
    } 
  }
    ...
    {currentUser &&
      <div className={style.primaryInfo}>
          ...
          {userId && <button className={classNames(buttonStyle.button, buttonStyle.shadow)}>
            ...
            <span className={buttonStyle.buttonText} onClick={writeMessage}>Написати повідомлення</span>
          </button>}
      </div>
    }
    </>
